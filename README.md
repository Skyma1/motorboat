...
| Мобильное       | React Native, Expo SDK 50, Expo Router               |
...
- **DISPATCHER** — управление причалами, ставки капитанов, просмотр рейсов и балансов
- **CAPTAIN** — создание/запуск/завершение рейсов, хозяйственные расходы
...
POST   /api/trips                — Создать рейс (Captain)
POST   /api/trips/:id/start      — Начать прогулку (Captain)
POST   /api/trips/:id/complete   — Завершить прогулку (Captain)
...
GET    /api/rates/dispatchers                    — Ставки диспетчеров (Admin)
PUT    /api/rates/dispatchers/:dispatcherId      — Обновить ставку диспетчера (Admin)
```
...
```
Баланс = Наличные от клиентов - Итоговая ЗП - Хозяйственные расходы
...
dispatcher_rates — Ставки диспетчеров
